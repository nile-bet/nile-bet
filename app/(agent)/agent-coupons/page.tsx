...
export default function AgentCouponsPage() {
  ...
  const handleLookup = async () => {
    ...
    const result =
      await lookupCouponByAgent(code.trim())
    if (result.success) {
  ...
  const handleApprove = async () => {
    ...
    const result = await approveCouponByAgent(
      code.trim(),
      user.id
    )
      ...
      <div className="bg-slate-dark border border-nile-blue/30 rounded-xl p-5 space-y-4">
        ...
        {lookedUp && (
          <div className="bg-nile-blue/20 border border-gold/30 rounded-xl p-4 space-y-3">
            ...
            {isInsufficient && (
              <div className="bg-nile-danger/10 border border-nile-danger/30 rounded-lg p-3">
                <p className="text-nile-danger text-sm font-semibold">
                ...
                <a
                  href="/agent-credits"
                  className="text-xs text-nile-orange hover:text-nile-orange/80 mt-2 block"
                >
